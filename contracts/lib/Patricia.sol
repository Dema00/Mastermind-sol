...
library Tree {
    ...
    function retrieve(Turn storage _turn, bytes8 key, uint8 guess_len ) internal view returns (bytes1 cc_nc){
        bytes8 node = _turn.nodes[_turn.root_node];
        uint8 key_explored = 0;
        ...
        while (
            ...
            if (
                bytes3(node) ^ mask == 0x0 &&
                 key_explored < guess_len
            ) {
